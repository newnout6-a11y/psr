...
class TestConversations:
    ...
    def test_add_message(self, tmp_db):
        ...
        result = tmp_db.add_conversation_message(conv_id, sender="customer", message_text="Привет!", platform_message_id="msg_1")
        assert result is True
        result2 = tmp_db.add_conversation_message(conv_id, sender="customer", message_text="Привет!", platform_message_id="msg_1")
        assert result2 is False

    def test_reopen_completed_conversation(self, tmp_db):
        conv_id = tmp_db.get_or_create_conversation("proj_1", "kwork")
        tmp_db.update_conversation_status(conv_id, "completed")
        tmp_db.add_conversation_message(conv_id, sender="customer", message_text="Новое сообщение", platform_message_id="msg_2")
...
class TestKworkExtensions:
    ...
    async def test_get_categories(self, mock_api):
        ...
        mock_api.request.return_value = {"response": [{"id": 11, "name": "Scripts", "categories": [{"id": 111, "name": "Python"}]}]}
...
class TestMarkResponse:
    def test_mark_response_records_audit(self, tmp_db, mock_project):
        ...
        tmp_db.save_proposal(
            project_id="test_123", platform="kwork", title="Test",
            proposal_text="Отклик", candidate_id=cid,
        )
        tmp_db.mark_response("test_123", "Ответ от клиента")
        with tmp_db._connect() as conn:
            row = conn.execute(
                "SELECT action FROM candidate_actions WHERE action = 'client_responded'"
            ).fetchone()
    ...
    def test_mark_response_updates_correct_proposal(self, tmp_db):
        ...
        tmp_db.save_proposal(
            project_id="proj_1", platform="kwork", title="V1",
            proposal_text="text1",
        )
        tmp_db.save_proposal(
            project_id="proj_1", platform="kwork", title="V2",
            proposal_text="text2",
        )
...
class TestAccountHealthMonitor:
    ...
    def test_busy_risk_warning(self):
        ...
        m = AccountHealthMonitor()
        m._cache = {
            "username": "u", "level": "L", "rating": 5, "reviews_count": 10,
            "connects_free": 5, "success_rate": 90, "active_orders": 6,
            "busy_risk": True, "captcha_required": False,
        }
        text = m.get_summary_text()
...
class TestTemplatesNoMarkdown:
    def test_templates_have_no_markdown_after_cleaning(self):
        ...
        portfolio = {"developer": {"experience_years": 4}, "cases": [{"title": "C1", "tech": ["python"], "description": "d", "result": "r"}]}
